...
<div class="grid max-w-screen-xl px-4 pb-8 mx-auto mt-16">
    <div class="grid col-span-full">
        ...
        <div class="mt-6">
            <x-primary-button>
                <a href="{{ route('tasks.create') }}">{{ __('messages.createTask') }}</a>
            </x-primary-button>
        </div>
        @endauth
        ...
        {{ Form::submit(__('messages.applyButton'), ['class'=>'inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md text-white font-semibold hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150']) }}
        </div>
        {{ Form::close() }}
            <table class="mt-4">
                <thead>
